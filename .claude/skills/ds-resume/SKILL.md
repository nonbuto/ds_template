---
name: ds-resume
description: 新しいセッションを開始するときは必ずこれを最初に実行する。SESSION.md・log.csv・FE_HYPOTHESES.md を読み込み「今どこにいるか・次に何をするか」を1画面で復元する。「前回どこまで進んだか」「次に何をすべきか」が不明なときも呼ぶ。作業を始める前の必須ルーティン。
---

# /ds-resume スキル

## このスキルの役割

新しいセッションを始めるとき、または文脈が途切れたと感じたときに呼ぶ。
複数のファイルを手動で読む代わりに、このスキルが「現在地」を1画面に集約する。

**いつ使うか:**
- 新しいチャットセッションを開始したとき
- 前回の会話内容が不明確になったとき
- 「今どこにいたっけ？」と感じたとき

---

## 実行手順

### フェーズ1: 状態ファイルの読み込み（実行層）

以下のファイルをすべて読み込む:

1. `SESSION.md` — 現在地スナップショット（最優先）
2. `experiments/log.csv` — 直近5件の実験記録
3. `FE_HYPOTHESES.md` — 仮説のステータス集計
4. `COMPETITION.md` — コンペ基本情報（Stage 0 の記録）
5. `TODO_TEMPLATE.md` — HIGH優先度の未完了項目

**逐次記録の実効性チェック（必須）:**

```bash
git status --short
```

- `experiments/runs/` に未追跡（`??`）の実験スクリプトがある場合: **前セッションで「1実験1コミット」が途切れたサイン**。
  ブリーフィング冒頭に以下を表示する:
  ```
  ⚠️ 未コミットの実験スクリプトがあります: <ファイル一覧>
  対応する log.csv 行・OOF 結果は記録済みですか？ 先にコミットを消化してから再開しましょう。
  ```
- log.csv の最新行に `oof_score` があるのに commit が無い場合も同様に警告する
- クリーンな場合: そのままフェーズ2へ

### フェーズ2: ブリーフィングの出力（実行層）

以下の形式で現在地を出力する:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 /ds-resume — セッション復元ブリーフィング
 （SESSION.md + log.csv より自動生成）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【現在のステージ】
  Stage X — <ステージ名>
  状況: <SESSION.md の「フェーズ内の状況」>

【スコア状況】
  | 指標 | OOF tuned | LB | OOF-LB乖離 | 実験ID |
  | ベスト | <値> | <値> | <±値> | exp<NNN> |
  ※ 乖離が大きい場合（>0.001）はその原因を「未解決の問い」に記載

【直近の実験（最新3件）】
  exp<NNN> | <モデル> | OOF=<値> | LB=<値> | <1行サマリー>
  exp<NNN> | <モデル> | OOF=<値> | LB=<値> | <1行サマリー>
  exp<NNN> | <モデル> | OOF=<値> | LB=<値> | <1行サマリー>

【FE仮説の状態】
  ✅ 採用: N件  ❌ 棄却: N件  🔵 未検証: N件
  次の未検証候補: H-<NNN> <タイトル>

【前回セッションで決めた次のアクション】
  1. <SESSION.md の「次にやること」より>
  2. ...

【未解決の問い・ブロッカー】
  - <SESSION.md より>

【今サイクルの重要方針】
  - <SESSION.md の「重要な方針」より>

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
続けますか？ 上記の「次のアクション」から始めてよいですか？
（内容に変更があれば教えてください）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

### フェーズ3: 確認と接続（思考層）

ブリーフィングを出力した後:

1. **「前回の続きから始めてよいですか？」** と確認する
2. ユーザーが変更や補足を言ったら、SESSION.md の「次にやること」または「未解決の問い」を更新する
3. ユーザーが「はい」または「続けて」と言ったら、「次のアクション」の1番目を実行し始める

---

## 注意事項

- SESSION.md が存在しない（プロジェクト開始直後）場合は「SESSION.md がまだ作成されていません。`/kickoff` から始めてください」と案内する
- log.csv が存在しない場合は「実験記録がありません。最初の実験を始めましょう」と案内する
- FE_HYPOTHESES.md が存在しない場合は仮説セクションを省略する
- 状態ファイルの内容が古い可能性があることをユーザーに伝え、「追加の変更があれば教えてください」と促す

**SESSION.md オーバーフロー検知（必須チェック）:**

フェーズ1で SESSION.md を読み込んだ後、行数を確認する:
```bash
wc -l SESSION.md
```
- **80行超**: 蓄積が起きているサイン。ブリーフィング冒頭に以下を表示する:
  ```
  ⚠️ SESSION.md が XX 行あります（上限80行）。
  古いエントリを削除して整理しますか？
  ```
  ユーザーが「はい」と言ったら、完了済みエントリを削除して 80 行以内に収める
- **正常（80行以内）**: そのままブリーフィングに進む
