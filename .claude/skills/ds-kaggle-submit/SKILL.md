---
name: ds-kaggle-submit
description: Kaggleへの提出・LBスコア確認・OOF/LB比較・learning記録を1サイクルで完結させる。「提出したい」「LBを確認したい」「実験結果を記録したい」「学びをlog.csvに書きたい」という場面で呼ぶ。learning列が空欄のまま次の実験へ進むことを防ぐ。
disable-model-invocation: true
---

# /ds-kaggle-submit スキル

## このスキルの役割

提出を「実験サイクルの完結点」として扱う。
何を変えたか → 提出 → OOF/LB比較 → 学びの言語化 → 次の仮説更新、
という流れを一連で行い、「提出したけど何も学ばなかった」を防ぐ。

---

## 実行手順

### フェーズ1: 提出前の文脈確認（思考層）

1. `experiments/log.csv` から今回の実験の `experiment_question`（目的）を表示する
2. 「今回の変更を1文で説明してください」とユーザーに問う
   - 目的に対して変更が整合しているか確認する
   - ズレがある場合は「実験目的は〈X〉でしたが、今回の変更〈Y〉はそれに対応していますか？」と確認する
3. 現在のOOF変化量を確認して提示する（前回比）

### フェーズ2: 提出実行（実行層）

> 提出ファイルは必ず `submission_path()` で生成する。命名規約は `CONVENTIONS.md#提出ファイルの命名規約`。

1. `git status --short` 確認
   - working treeが汚れている場合: 「未コミットの変更があります。コミット後に提出してください。」と表示して停止

2. **提出枠の状況を確認して提示する（UTC 基準・実結果からの逆算）**

   > **提出枠のリセットは UTC 00:00（JST 09:00）**。ローカル日付でカウントすると実態とズレる。
   > 自己申告カウント（state/SESSION.md の記録）に頼らず、必ず `kaggle competitions submissions` の実結果から逆算する。

   ```bash
   # 当日（UTC日付）の提出回数を実結果から逆算
   kaggle competitions submissions -c <COMPETITION> | grep $(date -u +%Y-%m-%d) | wc -l
   # コンペの締め切りを確認
   kaggle competitions list --search <COMPETITION>
   ```
   以下の形式で現状を提示する（タイムゾーンを必ず明記）:
   ```
   ── 提出枠の状況 ──────────────────────
   本日使用済み: X / N 回  （N は `src/config.py` の `DAILY_SUBMISSION_LIMIT`。残り Y 回、UTC日付基準・実結果から逆算）
   枠リセット: UTC 00:00（JST 09:00）
   コンペ締め切り: YYYY-MM-DD HH:MM UTC（JST: MM-DD HH:MM）
   残り日数: N 日
   推定残り総提出枠: Y + (N-1) × 5 = Z 回
   ──────────────────────────────────────
   未提出の候補ファイル（スコア順）:
     sub_171_lgb_cb_blend_0.91777_...csv
     sub_169_lgb_orig_proba_0.91760_...csv
     ...
   ```
   > **残り枠は使い切ることを基本方針とする。**
   > 未提出のまま締め切りを迎えた提出枠はゼロ価値。
   > ただし本日の枠が残り1回の場合は「最も有望な候補1本に絞る」判断を促す。

3. **⚠️ 実行前の最終確認（省略禁止）**

   以下の形式でユーザーに確認を取る。**ユーザーの「よろしいですか？」への明示的な返答を受けてから実行する:**

   ```
   `sub_NNN_model_0.XXXXX_YYYYMMDD_HHMM.csv` を提出します。
   本日 X/N 回目の提出になります。よろしいですか？
   ```

   - ユーザーが事前に「提出します」「試します」と言っていた場合でも、この確認は**省略しない**
   - 「提出します」= ユーザーの意思表示。AIの実行許可ではない
   - ユーザーの「OK」「はい」「どうぞ」を受けてから以下を実行:

   ```bash
   kaggle competitions submit -c <COMPETITION> -f data/output/submissions/<ファイル名> -m "<変更の説明>"
   ```

4. 数分後にLBスコアを確認する
   ```bash
   kaggle competitions submissions -c <COMPETITION> | head -5
   ```

5. **本日の残り枠がある場合**: 「あと Y 回提出できます。未提出の候補はありますか？」と確認する

### フェーズ3: OOF/LB 比較と振り返り（思考層）

スコアが判明したら以下を提示する:

```
今回の実験結果
  OOF変化:     <前回比 ±X>
  LB変化:      <前回比 ±Y>
  pub_oof_gap: <LB − OOF>（正=LBがOOFより高い, 負=OOFがLBより高い）
  現在ベストLB: Z
```

**⚠️ CV内部診断（OOF↔LB の2軸だけで判断しない。CLAUDE.md `G-DIAG`）**

`experiments/log.csv` の `cv_train_mean` / `cv_val_mean` / `cv_val_std` を読み取り、
**ユーザーが指摘する前に AI 側から**以下を併記する:

```
── CV内部診断 ──────────────────────
  train − val 乖離: <cv_train_mean − cv_val_mean>
  val − LB   乖離: <cv_val_mean − submit_score>
  fold間 val std  : <cv_val_std>
  → ΔOOF(<前回比>) は val std を <上回る / 下回る>
     ※ 下回る場合、その差は「測れていない」（順位はfoldの引き直しで入れ替わる）
```

**乖離パターンから原因を切り分ける** → 判定表と多クラス時の追加チェック（予測クラス分布 vs 実分布）は
`CLAUDE.md` の `G-DIAG` に従う。この診断が `class_weight="balanced"` の発見につながった実例は
`PLAYBOOK.md#教訓アーカイブ実測値つき` の L-01。

**伸びしろの所在の提示（毎回、スコア提示と同時に実施）:**

自前ベストLBと「LBトップ」の差分を示し、その差を埋める最有力候補を1行で提示する。
「今どこまで来ていて、あと何をすれば上に行けるか」を常に可視化する。

```bash
# LBトップ付近のスコアを確認
kaggle competitions leaderboard -c <COMPETITION> --show | head -5
```

```
── 伸びしろの所在 ────────────────────
自前ベストLB:  <自前Z>
LBトップ:      <トップ値>
差分:          <トップ − 自前Z>
最有力の埋め方: <アーキテクチャ乗り換え / 未試行FE / アンサンブル のどれか1つ>
  根拠: <なぜそれが差を最も埋めそうか。上位解法の傾向・自前の未探索次元から>
──────────────────────────────────────
```

**「最有力の埋め方」の判断指針（優先順）:**
1. **アーキテクチャの差**: 上位が自前と異なる主軸（RealMLP/stacking/NN等）を使っているなら、それが最大候補。`/ds-kaggle-research` で確認済みの上位アーキテクチャ分布を参照する
2. **未試行の情報次元**: 自前がまだ触れていないシグナル（外部データ・ドメイン特化指標）があるなら次点
3. **アンサンブル余地**: 上記2つが飽和し、多様なモデルが揃っているならブレンド

> **重要（Public LB追従との区別 — `G-NOISE` / `G-OOF` / `G-TWOAXIS`）**: ここで埋めるべきは
> 「構造的な伸びしろ（アーキテクチャ・情報次元・多様性）」であって Public LB スコアそのものではない。
> 「最有力の埋め方」は必ず上記3カテゴリの**構造的改善**を指す。Public LB を +ノイズ床だけ
> 上げる局所FEは「埋め方」に含めない。差を埋めた結果は **OOF でも改善していること**を確認する
> （OOF が動かず Public だけ縮む差分は Public 過適合の疑い）。

> **目的**: 「差分の所在」を毎回明示することで、局所改善（+ノイズ床のFE）に没入して
> 最大の伸びしろ（別アーキテクチャ等）を見落とすことを防ぐ。
> 差分が大きい（例: AUC で +0.005 以上）のに局所FEを繰り返している場合、
> 「今の角度では差は埋まりません。<最有力候補>に着手しませんか？」と能動的に提案する。

**pub_oof_gap 監視（CLAUDE.md `G-TWOAXIS`）:**

`experiments/log.csv` の全提出の pub_oof_gap 中央値（基準線）と比較する:
- 基準線 + 0.0005 を超えた場合: 「⚠️ Public 過剰浮上警告: pub_oof_gap が基準線を超えています。OOFを犠牲にした gap 拡大でないか確認してください」と state/SESSION.md に記録する
- 「OOF↓ かつ LB↑」パターン: 外部データ由来 FE を除き原則棄却を推奨する（OOF を犠牲に gap を操作しない）

**Plateau 検出（強制）:**

提出後、`experiments/log.csv` の直近 5 件の `submit_score` を確認する。
**同一 LB ± 0.00002 で 5 回以上提出が続いている場合、強制的に Plateau モードに入る**:

```
⚠️ Plateau 検出: 直近 5 提出が <LB値> ± 0.00002 に集中しています。

強制 brainstorm を実施します:
1. 未試行情報次元を 5 個以上列挙してください:
   (例: 天候、地域、時刻、外部統計、ドメイン特化指標)
2. `data/external/` の `保留` 判定ファイルを列挙し、再評価:
   ls data/external/
3. Kaggle Discussion / 上位 Notebook を調査:
   kaggle kernels list --competition <id> --sort-by voteCount --page-size 10

注: 「飽和」「全方向探索済み」は禁句。代わりに「現在の角度では伸びていない」と表現すること。
```

ユーザーの回答を受けて、新しい実験方向を確定してから次のステップへ進む。

**OOF/LBの差分パターンに応じて問いかけを変える:**

| パターン | AIのコメントと問い |
|---|---|
| OOF↑ & LB↑ | 「OOFの改善がLBに反映されました。変更は汎化しています。なぜ汎化したと思いますか？」 |
| OOF↑ & LB→（横ばい） | 「OOFは改善しましたがLBに反映されていません。過学習またはCV設計のズレが考えられます。心当たりはありますか？」 |
| OOF↑ & LB↓ | 「OOFが改善してもLBが悪化しました。今回変えた〈X〉が汎化を損なっている可能性があります。なぜそうなったと思いますか？」 |
| OOF→ & LB↑ | 「OOFは変わらずLBが改善しました。OOFが捉えていなかった何かをLBが見ている可能性があります。」 |
| OOF↓ & LB↓ | 「両方悪化しました。〈撤退基準〉に達していますか？次のアクションを決めましょう。」 |

**学びの言語化:**
「この実験から何を学びましたか？次に活かせることを1〜2文で教えてください。」

アンサンブル実験で改善がなかった場合は、以下の棄却分析を追加で実施する:

「効かなかった理由はどれに当てはまりますか？」
- **同一予測（相関 ≥ 0.998）**: 「どうすれば予測の多様性を生み出せますか？」
- **誤差の方向が同じ**: 「どのサンプルで既存モデルと同じ間違いをしていますか？」
- **全モデルで改善なし**: 「これはFEに戻るシグナルですか？それとも問題の上限に近づいていますか？」

→ 棄却理由と次の方向性をセットで `learning` 列に記録する

→ ユーザーの回答を `experiments/log.csv` の `learning` 列に記録する

### フェーズ4: 次サイクルへの接続（思考層）

1. `log.csv` の該当行を更新する:
   - `submit_score` = LBスコア
   - `lb_rank` = 順位
   - `oof_lb_gap` = `oof_score` − `submit_score`（正=OOF過大評価、負=OOF過小評価）
   - `learning` = ユーザーが語った学び

2. **`learning` は必ず記入してから終了する。空欄のまま次の実験に進まない。**
   - 記入が難しい場合は以下の問いかけで引き出す:
     - 「OOFの変化（Δ〈X〉）はLBに反映されましたか？されなかった場合、なぜだと思いますか？」
     - 「この実験で『予想通り』だったこと・『予想外』だったことは何ですか？」
     - 「次の実験の方針はこの結果で変わりましたか？どう変わりましたか？」
   - 一言でもよい。「OOF↑LB→。CV設計のズレか外れ値の影響を次に確認する」でも記録として十分。

3. 学びを踏まえて問いかける:
   「この学びを踏まえると、次に検証すべき仮説は何でしょうか？」

4. **`state/SESSION.md` を更新する**（セッション引き継ぎのため）
   - `最後に完了したこと`: 実験ID・LBスコア・oof_lb_gap・learning を記録
   - `次にやること`: ユーザーが答えた「次に検証すべき仮説」を記載
   - `現在の主要スコア`: 以下のテーブルを最新値で**上書き**（追記禁止）:
     ```
     | 指標 | OOF tuned | LB | OOF-LB乖離 | 実験ID |
     |---|---|---|---|---|
     | ベスト | 0.XXXXX | 0.XXXXX | ±0.XXXXX | expNNN |
     ```
   - `未解決の問い・ブロッカー`: oof_lb_gap > 0.001 の場合は原因を必ず記録
   - `今サイクルで決めた重要な方針`: この実験の学びから方針変更があれば追記

5. 次の実験の方向性が決まれば `/ds-new-experiment <次の実験名>` へ誘導する

---

### フェーズ5: Final 2 選定モード（最終日または残り 2 提出以下の時）

以下の条件のいずれかに該当する場合、フェーズ4 完了後に **Final 2 選定モード** に入る:

- コンペ締切まで残り 24 時間以内
- 残り総提出枠が 2 以下
- ユーザーが「Final 2 を決めたい」と明示的に依頼

**Step 0: コンペ戦略軸の再確認（Persona 投票の前に必須）**

`state/COMPETITION.md` の「コンペ戦略軸」セクション（`/ds-kickoff` Q7 で記録）を読み、ユーザーに再掲する:

```
/ds-kickoff で記録した戦略軸: <例: 自前モデルの限界追求>
この軸は今も有効ですか？（コンペ中に変わっていれば更新します）
```

- 戦略軸とスコア期待値が対立する候補がある場合（例: 外部知見系が Public 最高だが戦略軸は自前重視）、
  AI は **両論併記** する: 「スコア期待値軸の推奨: <候補X>／戦略軸に沿った推奨: <候補Y>」
- どちらを取るかは **ユーザーの決定**。AI がスコア期待値だけで推奨を一本化しない
- 最終決定と「どの軸を優先したか」を state/SESSION.md の「Final 2 確定」記録に含める

**Step 1: 候補プールを拡張する（`G-CEILING`）**

プール構築ルールは `CLAUDE.md` の `G-CEILING`（和集合方式）に従う。Public ベースのみのスクリーニングは禁止。
`experiments/log.csv` の `submit_score` / `oof_score` をそれぞれ rank 化し、どちらかが Top-10 の行を抽出して
`experiment_id / oof_score / submit_score / oof_rank / public_rank` を一覧提示する。

各候補を 4 プロファイルに分類して提示する:

| プロファイル | 意味 | Final 2 での扱い |
|---|---|---|
| OOF Top + Public Top | 標準候補 | 1 本目候補 |
| **OOF only Top** | Public sampling で過小評価 | ⭐ 必ず Persona 投票対象に |
| **Public only Top** | Public 過適合の可能性 | ⚠️ hedge を必ず付ける |
| Public LB +1σ 微改善 | ノイズ床近辺（`G-NOISE`） | ⚠️ 「突破」と呼ばない、保留扱い |

**Step 2: 原則の適用チェック（各候補について宣言する）**

- `G-NOISE`: Public LB 改善が評価指標別ノイズ床を超えているか
- `G-OOF`: OOF と Public が乖離している候補を見落としていないか
- `G-CEILING`: プールが和集合になっているか／**天井帯なら「単一最良の選定」ではなく集約（重み bagging・上位 N 平均）に切り替えたか**
- `G-OVERFIT`: 「Private 過適合候補」マーク済みの blend を Final 1 に置いていないか

**Step 3: 2 本目の価値を先に計算する（Persona 投票の前）**

`E[max(A,B)] − E[A]` を計算し、**ノイズ床未満なら 2 本目の選定に時間をかけない**（`G-CEILING`）。
→ 手順とコードは `PLAYBOOK.md#天井帯での意思決定ツールキット`

**Step 4: 9-Persona 投票**

→ 9 ペルソナの主張一覧・投票ルール・典型 Final 2 構成パターン・BoB の罠は
`PLAYBOOK.md#final-2-候補プールとpersona-投票` を Read して実施する。

進行:
1. AI が Final 2 候補（3-5 個）を一覧化する
2. 各 Persona の主張を簡潔に提示する
3. AI が投票を集計し推奨ペアを提示する（同数なら Risk Management を優先）
4. **ユーザーが最終決定する**（多数派と異なる場合は理由を明示）
5. 結果と「どの軸を優先したか」を state/SESSION.md「Final 2 確定」に記録する

> **目的**: 主観バイアスや hindsight bias を排除し、複数視点で robust な選定を行う。
> Public LB 最高への執着で Private LB shakedown を起こすパターンを防ぐ。

**Step 5: Kaggle Web UI で Final Submission を選択する（CLI 不可・省略厳禁）**

**Kaggle API / CLI には最終提出 2 本を指定する機能が無い。** Web UI の「Submissions」タブで
該当する 2 件のチェックボックスを手動で選択する必要がある。

```
Final 2 が確定しました。最後に Kaggle の Web サイトで選択してください（CLI では不可）:
  https://www.kaggle.com/competitions/<slug>/submissions
  1 本目: <ファイル名>
  2 本目: <ファイル名>
締切を過ぎると選択できません。選択が完了したら教えてください。
```

- **締切の数時間前には必ずこの案内を出す**（「Final 2 は決めたが UI で選択し忘れた」は致命的）
- ユーザーから完了の返答を得るまで、このステップを完了扱いにしない

**Step 6: 計画したが未実施のまま終わった項目を棚卸しする**

コンペ中に「試す価値がある」と判断しながら優先順位の都合で実施しなかった項目を一覧化し、
`docs/TODO_TEMPLATE.md` に記録する。コンペ終了後に上位解法と突き合わせて「その判断は妥当だったか」を
答え合わせするための材料になる。

> 過去コンペでは `10-fold CV パイロット` を計画に挙げながら最後まで実施せず、終了後に
> 上位解法（25 位）が 10-fold を使っていたことが判明した。**未実施項目を記録していなければ
> 「何を試さなかったか」すら思い出せない。**

---

## 注意事項

- このスキルは**ユーザーが明示的に呼び出した場合のみ**実行する
- `kaggle.json` が `~/.kaggle/` に存在することが前提
- フェーズ3の問いかけは省略しない。スコアだけ記録して終わることを防ぐのがこのスキルの存在意義
- **`learning` 列が空欄のまま次の実験に進むことを許可しない**
- **フェーズ5 (Final 2 選定モード) は最終日に必ず実施**。Persona 投票を省略しない

---

## 伸びしろの所在（提出のたびに提示する⑥）

> **⑥を毎回明示する理由**: 局所改善（+ノイズ床の FE）に没入して最大の伸びしろ（別アーキテクチャ等）を見落とすのを防ぐ。自前ベスト LB と LB トップの差＋それを埋める最有力候補（アーキテクチャ乗り換え / 未試行 FE / アンサンブルのどれか 1 つ）を 1 行で示す。差分が大きいのに局所 FE を繰り返している場合、AI は「今の角度では差は埋まりません。〈候補〉に着手しませんか？」と**能動的に提案する**。

---

## 提出枠の管理方針

**残り枠は使い切る。未使用の提出枠はゼロ価値。**「何を Submit するか」は固定配分ではなく、
実験の進行状況と残り枠を見て「**今この瞬間に最も価値のある 1 本**」を毎回選ぶ。

**カウントは UTC 基準**（枠リセットは UTC 00:00 = JST 09:00）。自己申告に頼らず
`uv run python -m scripts.harness.deadline_status` で確認し、state/SESSION.md・log.csv の日時には
タイムゾーンを明記する。**30 分超のジョブを始める前と、前回の時刻確認から 30 分以上
経っている場合は必ず再実行する**（古い残り時間の見積もりで長時間ジョブを開始する事故を防ぐ）。

---

## 提出のたびに提示する 6 項目

`/ds-kaggle-submit` のたびに提示する: ①本日（UTC）の使用済み / 上限 ②締切までの残り日数 ③推定残り総枠 ④`data/output/submissions/` の未提出候補 ⑤**OOF-LB 乖離**を state/SESSION.md のスコアテーブルへ追記 ⑥**伸びしろの所在**。

> ⑥の趣旨と文面は `.claude/skills/ds-kaggle-submit/SKILL.md`（伸びしろの所在）を参照。

---

## Final 2 の選定原則（詳細）

> 2 本目を育てる手順は `PLAYBOOK.md#小規模アンサンブルfinal-2-の-2-本目-hedge-候補を育てる手順`、
> 効かなかったときの次アクションは `PLAYBOOK.md#アンサンブル棄却分析`。
> 派生アンサンブルの予測ファイルは `CONVENTIONS.md#oof-test-予測ファイルの命名規約派生アンサンブルの識別`
> に従って `_ens_` を含める（自己参照リークの防止）。

「Public 最高を 2 つ」より **Private shakedown 防御**を最優先する（Public は全テストの 30% 程度で評価され、±0.00005 の差はノイズ範囲内）。選定原則を優先順に:

1. **コンペ戦略軸の再確認** — Kickoff で記録した戦略軸（state/COMPETITION.md）を再掲してから議論を始める。スコア期待値と戦略軸が対立する場合は**両論併記し、ユーザーが決定する**（AI がスコア期待値だけで推奨を一本化しない）
2. **候補プールの作り方** — Public Top のみのスクリーニングは禁止。和集合の取り方は CLAUDE.md `G-CEILING`、プロファイル表の様式は `PLAYBOOK.md#天井帯での意思決定ツールキット`
3. **Public 最高 1 本 + 構造的多様性を持つ hedge 1 本** — ただし 2 本目の価値は先に `E[max]` で計算する（→ `G-CEILING`）
4. **Shakedown 防御** — 両者が同じ family なら片方コケて共倒れ。親が独立な 2 blend を取る
5. **9-Persona 投票** — 主観バイアスを排除するため複数視点で評価
