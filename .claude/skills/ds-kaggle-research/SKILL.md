...
以下のフェーズ0-2 は**大量のノートブックを読む**。親の文脈を埋めないよう、
サブエージェント `kaggle-researcher` に隔離して実行する。

1. **ユーザーに確認する** —— 「`kaggle-researcher` で調査しますか？」（AI の裁量では起動しない）
2. 承認を得たら 1 本起動し、調査対象（コンペ slug・知りたいこと）を渡す
3. 返ってきたアーキテクチャ分布・接続案・未検証事項を、**親が `state/KAGGLE_RESEARCH.md` へ追記**する

手順の詳細（以下のフェーズ）はエージェント側にも同じ内容が入っている。
自分で調べる場合はそのまま以下に従う。
