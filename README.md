# DS Template v5 — Kaggle Competition Workspace
...
- CLAUDE.md 指針#9 に**失敗の履歴そのもの**を記載（3度目を防ぐため）

### ② 天井帯での意思決定 — 「当てにいく」のをやめ「集約する」

候補間の差がノイズ床に近い領域では、OOF も Public も**上位の順序を予測できない**（実測: OOF 上位10件 ∩ Private 上位10件 = **1件**）。

- **指針#27**: 天井帯では単一最良を選ばず**集約**する（重み探索の bagging / 上位 N 構成の平均）。実測で Private 最高を記録したのは「12 シードの重み探索を平均した」構成だった
- **指針#28**: **情報天井の判定**手順（argmax 一致率 + セグメント別分解）と、判定後のアーキテクチャ探索の打ち切り
- **指針#18 に但し書き**: 「OOF で足切りする」は有効／「OOF で 1 位を選ぶ」は無効。機械的に OOF 最高を選ぶと最悪手になりうる（実測: OOF 最高候補の Private は天井帯でも下位）
- PLAYBOOK に手順6（集約への切り替え）・手順7（情報天井の判定）を追加

### ③ CV 内部診断を常設の判断軸に（指針#31）
...
- **指針#13 にチェック4「不公正比較チェック」**を追加（HP / 特徴量セット / CV 条件 / 較正の4項目）
- **特徴量数が ±20% 以上変動した場合の棄却は、HP 再調整前は「棄却」として記録してはならない**（暫定保留とする）
- **指針#29**: 提出枠を「良い候補を出す」だけでなく**「判断基準そのものを検証する」**ためにも使う（1日1枠を検証提出に）

### ⑤ 探索の方向づけ — 情報源の多様性とアーキテクチャの多様性は違う

- **指針#8 追記**: 情報天井が実証されたときこそ、AI は「探索を縮小する提案」ではなく**「問いを立て直す提案」**を出す（Kickoff 前提の再検証 / 棄却済み仮説の再評価 / 外部調査のやり直し / 評価指標の性質の測り直し）
- **指針#12 強化**: 「未試行の情報次元」5個のうち**最低2個は現データに存在しない変数**でなければならない（既存変数の組み合わせで枠を埋めることを禁止）
- **指針#10 強化**: 1つの外部データの否定的結果を全体に一般化しない／**行レベル照合で「同じ生成プロセスのデータか」を最初に確認する**
- **指針#23 の適用範囲拡張**: 量子（1行あたりの指標変化）の計算を、目標設定だけでなく**LB 観察の解釈**にも必須化
...
- **指針#30**: 技術的ブロッカーを「回避できたから解決」としない運用 + PLAYBOOK に「既知の落とし穴（ライブラリ別）」の章を新設
