...
| 項目 | 現在値 |
|---|---|
| 毎セッション自動ロード（CLAUDE.md） | **3,545 字（-94%）**（59 行。上限は 3,000〜5,000 字かつ 60 行 / v5 は約 56,000 字） |
...
| ハーネスのテスト | **227 件**（うち `slow` 2 件: e2e パイプライン / 変異注入）。`uv run pytest` |
...
│   └── utils/             # finalize / foldcache / multiseed / ensemble / csvlock / encoders / pseudo / postprocess / plot_style
│
├── experiments/
│   ├── log.csv            # 全実験サマリー
