...
### L-39 ガードが壊れたときにガードが消える —— fail-open の 3 つの入口

*対応する指針: `G-MECH`*

**教訓 (第7ラウンド・プロセス制御の検査)**: 提出ゲートは
「取り消せない・回数制限つき・外部に見える」唯一の操作を止める関門なのに、
**ゲート自身が壊れると素通りする**経路が 3 つ残っていた。

| 入口 | どう壊れるか | 結果 |
|---|---|---|
| **トップレベル import** | `src/config.py` を編集中・venv 破損・`uv sync` 途中 | main() に到達せず exit 1。**PreToolUse が止めるのは exit 2 だけ**なので提出が通る |
| **ラッパーのオプション** | `uv run --no-project` `timeout 60` `nice` `sudo -u` `env -i` `xargs -I{}` | コマンド位置の追跡がオプションを本体と誤認し、**行の残り全体を読み飛ばす** |
| **切り詰められた提出ファイル** | `to_csv` 中に落ちる | ゲートは行数を**表示するだけ**で `sample_submission` と突き合わせない（301 行が 38 行でも通った） |

**恒久対応**:
1. **ゲートが依存するものは関数内で import する。** トップレベル import は
   「壊れたら黙って消える」経路を作る。既存の try/except は import には届かない
2. **コマンド位置を追わない。** 全トークン位置で三つ組を探せば、
   **ラッパーの一覧を網羅する必要そのものが消える**。クォートの中身は
   shlex が 1 トークンに融合するので、誤検知は増えない
3. **不可逆操作の直前では、使える構造チェックを全部使う。** 行数を表示するだけでなく
   突き合わせる。成果物は tmp + `os.replace` で書き、途中の状態を「存在する」ようにしない

### L-40 常時走る仕組みは、止まり方まで設計する

*対応する指針: `G-MECH`*

**教訓 (同ラウンド)**: 30 秒ごと・毎ツール前に走るスクリプトは、
**1 回の詰まりが全体を止める**。同時に、書きかけの状態を人に見せてもいけない。

| 症状 | 実測 |
|---|---|
| stdin が閉じられないと無限に待つ | `isatty()` だけでは不十分。パイプが開いたままだと `read()` が戻らない |
| ハートビートの非原子書き込み | 読み取り **77,054 回中 1,807 回（2.3%）がパース失敗** |
| 死んだ実験の状態ファイルが残る | 自動掃除が無く、`💀残骸` を人が消すまで出続ける |
| クラッシュした実験の台帳行 | **永久に回収されず**、落ちるたびに ID が飛ぶ |
| Optuna study の初回並行作成 | DB が無い状態から 4 並行 → **1〜3 個が即死**（trial が走る前） |
| テストの複製が別 venv を含む | `.venv` の完全一致で `.venv-autogluon`（580 MB）を毎回コピー。残骸 20.8 GB |

**恒久対応**: stdin は `select` で期限つきに。状態ファイルは tmp + `os.replace`。
実験の開始時に死んだハートビートを掃除。台帳の回収条件から「目的が書かれていること」を外す
（クラッシュ行には目的が無い）。並行で競合する初期化はリトライする。
除外パターンは前方一致（`.venv*`）にし、複製は `try/finally` で必ず消す。

### L-41 テンプレートでは「未使用」より「見つけられない」が問題

*対応する指針: `G-MECH`*

**教訓 (第7ラウンド・統合作業)**: 「個別コードの最適化になっていないか」という指摘を受けて
棚卸ししたところ、**公開ヘルパー 61 件のうち 20 件に到達経路が無かった**。

ただし普通の意味の「デッドコード」とは違う。テンプレートの `src/utils/` は
**利用者のコンペコードから呼ばれる道具箱**なので、`src/` 内に呼び出しが無いのは正常。
問うべきは「**利用者がその存在に辿り着けるか**」:

1. `scripts/` から呼ばれる実行経路があるか
2. 文書に名前が出ているか
3. `src/` の他モジュール（または同ファイルの公開関数）が使っているか

どれも無ければ、**作ったのに知られないまま埋もれる**。

実際に見つかったもの:

| 対象 | 処置 |
|---|---|
| `src/utils/logger.py`（`get_logger`） | **削除**。全モジュールが `print` を使っており、一度も呼ばれていなかった |
| `validate_class_balance` | `run_all_validations` に**配線**（分類のときだけ）。書かれていても呼ばれない検証は「検証がある」という誤った安心を作る |
| `validate_feature_ranges` | 期待範囲を引数で要するので自動化できない。**呼び方をコメントで明示** |
| `add_count_encoding` | 文書に 1 行追加（target を使わないので前処理で 1 回でよい） |

**恒久対応**: 一度きりの掃除で終わらせず、`doc_audit` の **C17** にした。
「作ったが導線が無い」が次から機械で見つかる。
検知テストつき（到達経路の無い公開関数を足すと鳴ることを確認）。

**あわせて、選択肢そのものを減らした**（L-41 の前半）:
アンサンブル API 9 個のうち 4 個が誰からも呼ばれておらず、残りも
「どれを使えばいいか」が伝わらなかった。死蔵を削除し、**順路を 1 本道**
（相関確認 → hillclimb → signed_stack）で示した。
**選択肢が多いこと自体がコスト** —— 次のコンペで迷う時間は、規律の緩みと同じくらい高くつく。

### L-42 「コストが安いガード」は、何もしていないから安いのかもしれない

*対応する指針: `G-MECH`*

**教訓 (第7ラウンド・費用対効果の測定)**: 規律機構の維持コストを測ったところ、
**驚くほど安かった**。

| 機構 | 1 回 | 発火数 | 累計 |
|---|---|---|---|
| PreToolUse（提出ゲート） | 32 ms | 1,341 | 43 秒 |
| PostToolUse（可視化ガード） | **8 ms** | 1,319 | **10 秒** |

しかしこれは良い知らせではなかった。**PostToolUse が 8 ms なのは、
20 秒の時間窓を外れて Python を起動していなかったから**。
`log.csv` の最終更新は 26.8 時間前で、**1,319 回の発火機会に対して
一度も実際には走っていなかった**。
...
| シナリオ | 20 秒窓 |
|---|---|
| 前景で学習 → Bash 終了直後に log.csv 更新 | 内（動く） |
| **背景で学習** → 完了時に Bash 呼び出しが無い | **外（動かない）** |
| 学習後に別作業を挟む → 次の Bash が 20 秒超 | **外（動かない）** |

ユーザーの運用は「昼夜を問わず長時間学習を背景で回す」なので、**主要な使い方で死んでいた**。

**恒久対応**: 時間窓をやめ、**「log.csv が前回検査より新しいか」**（マーカーファイルとの
`-nt` 比較）にした。背景実行でも次の Bash で必ず 1 回走り、変化が無ければ 4 ms で終わる。
実測: マーカーなし 676 ms（走る）→ 変化なし 4 ms → 更新後 697 ms（再び走る）→ 4 ms。

**測り方の教訓**: 最初「679 ms × 1,319 回 = 15 分の税」と見積もって報告しかけた。
実際には hook のシェル前置きが 8 ms で、Python は起動していなかった。
**コストを測るときは「1 回の実行コスト × 回数」ではなく、実際に走る条件を確かめる。**
そして**安すぎる数字が出たら、まず「本当に動いているか」を疑う**（L-25 の空洞化と同じ形）。

**参考: 文書系の検査は明確に元が取れている**（このセッションの実績）:
C11 が 4 回（テスト件数・チェック数のズレ）、C16 が 2 回（config の未記載）、
C17 が導入時に 20 件、C10 が 1 回（API 変更で孤立した節）、
C7 が 1 回（テンプレート本体へのコンペ識別子混入）。**すべて私の取りこぼしを捕まえた。**

### L-43 検査が `.md` にしか届いていなかった —— 最も人目に触れる文面が未検査だった

*対応する指針: `G-MECH`*

**教訓 (v7 完成検査)**: ハーネス視点で v7 全体を検証した結果、
**同じ検査が `.md` にしか届いておらず、`.py` の中の同種の誤りが素通り**していた。

| 検査 | 目的 | 届いていなかった先 |
|---|---|---|
| C14（文書中のコマンド） | `uv run python scripts/x.py` は `ModuleNotFoundError` になる | `.py` の docstring と **print 文** |
| C3（指針の ID） | v6.6 の `指針#N` → `G-*` 移行の取り残しを検知 | 同上 |
| C16（config の文書化） | 設定を足したのに書き忘れる | **逆向き**（config から消えたこと） |

とくに痛いのは C14。**可視化ガードがブロック時に印字する唯一の解除手順**が、
`uv run python scripts/feature_report.py`（＝動かない形式）のままだった。
**ガードが解けない手順を印字している状態は、ガードへの信頼を最初に壊す。**

C16 の片方向も実害があった。初期化スキルが config を「ブロックごと置換」と指示しており、
**指示どおりに適用すると 6 キーが消えて `ImportError: cannot import name 'GROUP_COL'`**
になる。C16 は「config にある名前が文書に出てくるか」しか見ないので**素通り**していた。

**恒久対応**:
1. C14 / C3 の走査対象に `src/**/*.py` と `scripts/**/*.py` を追加。
   走査を広げた瞬間に **12 件**が出た（すべて修正済み）
2. C16 に逆向きを追加 —— **コードが `from src.config import` している名前が実在するか**を
   AST で照合する。正規表現では他の import まで拾ってしまう
3. 初期化スキル（`ds-kickoff` と `~/.claude/skills/ds-kaggle-setup`）の指示を
   「該当行だけ書き換える」に変更し、**ブロックごと置換を明示的に禁じた**
4. 検知テストを 5 件追加（複製に誤りを注入して doc_audit が鳴ることを確認）
