...
> 実測 (L-06): 努力目標→条件明示の2世代とも形骸化し、2コンペ連続で「可視化が最初の数日に集中」が再発（総31枚中23枚が最初の4日） → `PLAYBOOK.md#教訓アーカイブ実測値つき`
...
> 実測 (L-08): 重み 6 個 → 13 個で **OOF +0.00004 なのに LB −0.00017**、gap が正から負へ反転。独立データ検証では +0.00013 → **−0.00021** と逆転した → `PLAYBOOK.md#教訓アーカイブ実測値つき`
